...
#====================================Ver 200925======================================#
#Semantic Segmentation Sensor 오랜 시간 실행시 Unreal Engine 튕김 현상 있어서 관련 코드 주석처리
#====================================Ver 201023======================================#
# Carla Version 0.9.10 대응 업데이트
# Semantic Segmentation Sensor 튕김 오류 해결
# carla 0.9.9에서 필요했던 camera exposure 삭제
# throttle 상승 속도 상향
# recording시 필요한 디렉토리 자동 생성기능 추가
# 다중 클라이언트 안정성 위해 맵 변경 기능 삭제/ Unreal Engine에서 변경 가능
# 기본 설정된 차량 변경(Carla의 numbering이 바뀜) (AUDI-A2)
#====================================Ver 201026======================================#
# Q를 사용하여 기어를 R로 변환하는 것이 아닌, S를 눌렀을 때 바로 후진이 되도록 변경
# throttle 속도 상향
#====================================Ver 201028======================================#
# IP주소 오류 고침
...
class World(object):
    ...
    def restart(self):
        ...
        #blueprint = self.world.get_blueprint_library().filter('walker.pedestrian.*')[0]
        ...
        self.camera_manager = Camera(self.player, self.hud, False)
    ...
    def set_synchronous_mode(self, synchronous_mode):
        ...
        settings.fixed_delta_seconds = 0.033333333
    ...
    def render(self, display, display2):
...
class Camera(object):
    def __init__(self, parent_actor, hud, record):
        self.sensor = None
        self.surface = None
        self._parent = parent_actor
        self.hud = hud
        ...
        self._camera_transforms = [
            ...
            carla.Transform(carla.Location(x=0.5, y=0, z=1.6)),      # Mid camera
            carla.Transform(carla.Location(x=0.5, y=-0.5, z=1.6)),  # Left camera
            carla.Transform(carla.Location(x=0.5, y=0.5, z=1.6))]   # Right camera
    ...
    def toggle_recording(self):
        ...
        if os.path.isdir("./seg"):
            ...
            os.mkdir("seg")
        if os.path.isfile("steer_angle_log.txt") & self.recording == True:
            os.remove("steer_angle_log.txt")
        #=============================================#
    ...
    def _parse_image(weak_self, image):
        ...
        # array = cv2.cvtColor(final_hsv, cv2.COLOR_HSV2BGR)
        #===================================================#

        array = array[:, :, ::-1]
        self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        
        if self.recording: #R 눌렀을 시 녹화 및 steering angle 기록
            array = cv2.cvtColor(array, cv2.COLOR_BGR2RGB)

            #==========================사용하지 마세요!================================#
            # semantic = image.convert(cc.Raw)
            # print("IMAGE CONVERT RESULT : "+str(self.sensors[self.index][1]))
            # semantic = np.frombuffer(semantic.raw_data, dtype=np.dtype("uint8"))
...
# ==============================================================================
# -- Semantic Manager ----------------------------------------------------------
# 연산량 줄이기 위해 따로 뺐음, Camera Manager와 통합 가능
# ==============================================================================
...
class Semantic_Camera(object):
    ...
    def set_sensor(self, index, notify=True):
        ...
        if needs_respawn:
            ...
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: Semantic_Camera._parse_image(weak_self, image))
        self.index = index
...
def game_loop(args):
    ...
    try:
        ...
        semantic_display = 1 #cv2.namedWindow('Semantic Segmentation')
        ...
        #=================Python self-driving agent====================#
        # 아래랑 같이 주석 해제해서 쓸 것
        # 고속 주행 가능 agents/basic_agents 에서 target speed 변경
        # 고속 주행시 장애물 회피가 잘 되지 않음
        #agent = BasicAgent(world.player)
        #agent = RoamingAgent(world.player) #주석처리 유지해야함
        #spawn_point = world.map.get_spawn_points()[0]
        #==============================================================#
        
        clock = pygame.time.Clock()
        #=========================미완성/사용시튕김==========================#
        #world.set_synchronous_mode(True)
        #================================================================#
        while True:
            clock.tick(30)  #Client FPS
